...
from backbone import (res32_cifar, res50, res18, res10, res101, resnet110, resnext50, efinet, afa)
...
class Network(nn.Module):
    ...
    def get_backbone_layer_info(self):
        if "cifar" in self.cfg.BACKBONE.TYPE:
            ...
            layers = 4
            blocks_info = [1, 1, 1, 1]
        elif 'res101' in self.cfg.BACKBONE.TYPE:
            layers = 4
            blocks_info = [3, 4, 23, 3]
        else:
            layers = 4
            blocks_info = [3, 4, 6, 3]
    ...
    def get_feature_length(self):
        if "cifar" in self.cfg.BACKBONE.TYPE:
            num_features = 64
        elif 'efinet' in self.cfg.BACKBONE.TYPE:
            num_features = 328
        elif 'res101' in self.cfg.BACKBONE.TYPE:
            num_features = 2048
        elif 'res10' in self.cfg.BACKBONE.TYPE:
            num_features = 512
        elif 'res18' in self.cfg.BACKBONE.TYPE:
            num_features = 512
        else:
            num_features = 2048
